...
/**
 * Per-project override of a shared image.
 ...
 * `file` is a content-relative path, e.g. `images/icons/logo.png`.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "Failed to fork image";
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "Failed to revert image";
    return NextResponse.json({ error: msg }, { status: 500 });
  }
}
